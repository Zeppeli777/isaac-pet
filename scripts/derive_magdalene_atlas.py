...
def lock_layout(head_faces_left: bool, source_x: int) -> tuple[int, bool]:
    ...
    if source_x in (64, 96):
        return (6, False) if head_faces_left else (2, False)
    return HAIR_INDEX_FOR_HEAD[source_x], head_faces_left
...
def composite_locks(
    ...
    scale: float,
    head_y: int,
    head_faces_left: bool,
    hair_source: Image.Image,
    isaac_source: Image.Image,
) -> tuple[Image.Image, dict[str, object]]:
    """Overlay the Golden Locks on one cell and report how they landed."""
    hair_index, mirrored = lock_layout(head_faces_left, source_x)
...
def derive_atlas(
    ...
    """Composite the Golden Locks onto every occupied cell of an Isaac atlas.

    Used for the main 8×11 sheet and for the shooting and vertical-walking helper
    atlases, which would otherwise keep Isaac's bare head under a role appearance.
    `row_faces_left` names the cells this atlas mirrors as a whole; the profile heads
    additionally follow the orientation the head match itself reported.
    """
    derived = base.copy()
    ...
    for row, column, origin, cell, anchor in cells:
        ...
        overlay, application = composite_locks(
            cell,
            origin,
            row,
            column,
            ...
            scale,
            anchor["y"],
            anchor["mirrored"] if source_x in (64, 96) else row_faces_left(row, column),
            hair_source,
            isaac_source,
        )
    ...
    return derived, applications, low_confidence
...
def main() -> None:
    ...
    parser.add_argument(
        "--shooting-atlas", type=Path, default=ROOT / "Resources/shooting-atlas.webp"
    )
    parser.add_argument(
        "--shooting-output",
        type=Path,
        default=ROOT / "Resources/Agents/magdalene-shooting-atlas.webp",
    )
    parser.add_argument(
        "--vertical-walking-atlas",
        type=Path,
        default=ROOT / "Resources/walking-vertical-atlas.webp",
    )
    parser.add_argument(
        "--vertical-output",
        type=Path,
        default=ROOT / "Resources/Agents/magdalene-walking-vertical-atlas.webp",
    )
    ...
    # The helper atlases share the main sheet's cell size, so the same derivation runs
    # on them: without it the shooting and vertical-walking poses keep Isaac's bare head.
    # Each atlas names the cells it mirrors as a whole — the main sheet mirrors its left
    # walk and the lower direction row, the shooting sheet mirrors only its left column.
    def main_faces_left(row: int, column: int) -> bool:
        return row == 2 or (row == 10 and column >= 1)

    def shooting_faces_left(row: int, column: int) -> bool:
        return column == 3

    def vertical_faces_left(row: int, column: int) -> bool:
        return False

    helpers = {
        "shooting_atlas": (
            args.shooting_atlas,
            args.shooting_output,
            1,
            4,
            shooting_faces_left,
        ),
        "vertical_walking_atlas": (
            args.vertical_walking_atlas,
            args.vertical_output,
            VERTICAL_WALK_ROWS,
            VERTICAL_WALK_COLUMNS,
            vertical_faces_left,
        ),
    }
    helper_results: dict[str, dict[str, object]] = {}
    for name, (source_path, output_path, rows, columns, faces_left) in helpers.items():
        helper = Image.open(source_path).convert("RGBA")
        expected = (CELL_WIDTH * columns, CELL_HEIGHT * rows)
        if helper.size != expected:
            raise SystemExit(
                f"{name} must be {expected[0]}x{expected[1]}, got {helper.size[0]}x{helper.size[1]}"
            )
        derived, applications, low_confidence = derive_atlas(
            helper, hair_source, isaac_source, rows, columns, faces_left
        )
        ...
        contact = QA / f"{output_path.stem}-contact.png"
        make_contact_sheet(reloaded, contact)
        helper_results[name] = {
            "path": str(output_path.relative_to(ROOT)),
            "size": list(reloaded.size),
            "order": ["up-flat-back", "right-closed", "down-closed", "left-closed"]
            if name == "shooting_atlas"
            else ["down-front-head", "up-back-head"],
            "occupied_cells": len(applications),
            "low_confidence_head_matches": low_confidence,
            "hair_coverage_failures": [a for a in applications if not a["hair_covers_head"]],
            "applications": applications,
            "contact_sheet": str(contact.relative_to(ROOT)),
        }

    derived, applications, low_confidence = derive_atlas(
        base, hair_source, isaac_source, ROWS, COLUMNS, main_faces_left
    )
    ...
    for name, result in helper_results.items():
        coverage_failures += [
            {**item, "atlas": name} for item in result["hair_coverage_failures"]
        ]
    manifest = {
    ...
    for name, result in helper_results.items():
        print(
            f"Derived {name}: {result['path']} "
            f"({result['occupied_cells']} cells, {len(result['low_confidence_head_matches'])} low-confidence)"
        )
